...
func (cd *Connection) UpdateAccountRangeLevels(ID primitive.ObjectID, obj []model.Range) error {
	session, err := cd.connect()
	if err != nil {
		fmt.Println("Error while connecting to DB " + err.Error())
		return err
	...
	// id, _ := primitive.ObjectIDFromHex(ID)
	filter := bson.D{{"_id", ID}}
	update := bson.D{{"$set", bson.D{{"ratiodata", obj}}}}
	_, errWhenUpdate := c.UpdateOne(context.TODO(), filter, update)
	if errWhenUpdate != nil {
		logrus.Error("Error when updating the MassBalanceRanges in DB : " + errWhenUpdate.Error())
		return errWhenUpdate
	}
	return nil
}
