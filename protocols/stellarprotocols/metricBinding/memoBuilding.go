...
Fields
	...
	4. Future Use     - 14 bytes
*/

type MetricBinding struct{}

func (metric *MetricBinding) BuildMemo(mapMetricId uint64, tenantId uint32, noOfFormula uint16) (string, error) {
	...
	memo := stellarprotocols.UInt64ToByteString(mapMetricId) + stellarprotocols.UInt32ToByteString(tenantId) + stellarprotocols.UInt16ToByteString(noOfFormula) + string(decodedStrFetureUsed)
	if len(memo) > 28 {
