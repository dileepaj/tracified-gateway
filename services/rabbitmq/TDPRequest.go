...
func TDPRequestReceiverRmq() error {
	previousTxnHash := ""
...
func (AP *AbstractXDRSubmiter) TDPRequestSendToQueue(w http.ResponseWriter, r *http.Request,) error {
	rabbitConnection := `amqp://` + USER + `:` + PASSWORD + `@` + HOSTNAME + `:` + PORT + `/`
	conn, err := amqp.Dial(rabbitConnection)
	if err != nil {
	...
	json.NewEncoder(reqBodyBytes).Encode(AP)
