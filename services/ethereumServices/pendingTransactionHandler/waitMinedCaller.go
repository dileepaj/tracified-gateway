...
func CallWaitMined(client *ethclient.Client, tx *types.Transaction) (int64, error) {
	...
	if errInGettingReceipt != nil {
		...
		return -1, errors.New("Error in getting receipt: Error: " + errInGettingReceipt.Error())
	} else {
		...
		if receipt.Status == 0 {
			...
			if errorInCallingTransactionStatus != nil {
				...
				return int64(receipt.Status), errors.New("Transaction failed.")
			} else {
			...
			return int64(receipt.Status), errors.New("Invalid receipt status for 'WaitMined', Status : " + fmt.Sprint(receipt.Status))
		}
