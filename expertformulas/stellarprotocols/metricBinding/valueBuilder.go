...
func (metric *MetricBinding) BuildGeneralValueManageData(element model.ValueBuilder,formulaId string) (txnbuild.ManageData, string, []byte, error) {
	...
	if err != nil {
		logrus.Error("Error when converting stage ID " + err.Error())
		return txnbuild.ManageData{}, "", []byte{}, errors.New("error when converting stage ID " + err.Error())
	}
...
func (metric *MetricBinding) ValueDefinitionBuilder(element model.GeneralValueDefBuildRequest,formulaId string) (txnbuild.ManageData, string, []byte, error) {
	...
	futureUseInKey = strings.Repeat("0", 4)
	...
	if err != nil {
		return txnbuild.ManageData{}, "", []byte{}, errors.New("future used byte building issue in formula definition")
	}
