...
func PGPValidator(sha256hash, signature, originalMsg string) (error, bool,string) {
	object := dao.Connection{}

	// get the public key from the DB using its hash
	publicKeyDet, errWhenGettingPublicKey := object.GetRSAPublicKeyBySHA256PK(commons.ConvertBase64StringToHash256(sha256hash)).Then(func(data interface{}) interface{} {
		return data
	}).Await()
	...
	if verifiedPlainText == originalMsg {
		return nil, true,publicKeyData.ID.Hex()
	} else {
