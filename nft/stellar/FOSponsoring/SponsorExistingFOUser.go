...
var tx1 *txnbuild.Transaction
var decrpytIssuerSecretKey1 string

func SponsorNewFOUsers(txnPayload model.TransactionData) (string, error) {
	fmt.Println("----------------test 6")
	...
	if err != nil {
		...
		decrpytIssuerSecretKey1 = commons.Decrypt(Keys[0].AccountIssuerSK)
		fmt.Println("decryt issuer sk ", decrpytIssuerSecretKey1)
		if data == nil {
	...
	if txnPayload.Type == "0" {
		tx1, err = txnbuild.NewTransaction(
			txnbuild.TransactionParams{
				SourceAccount:        &sourceAccount,
				IncrementSequenceNum: true,
				Operations: []txnbuild.Operation{&beginSponsorship, &createAccount, &endSponsorship, &beginSponsorship, &manageDataType, &endSponsorship, &beginSponsorship, &manageDataIdentifier, &endSponsorship,
		...
		tx1, err = txnbuild.NewTransaction(
			txnbuild.TransactionParams{
				SourceAccount:        &sourceAccount,
				IncrementSequenceNum: true,
				Operations: []txnbuild.Operation{&beginSponsorship, &createAccount, &endSponsorship, &beginSponsorship, &manageDataType, &endSponsorship, &beginSponsorship, &manageDataToIdentifier, &endSponsorship,
					&beginSponsorship, &manageDataProductName, &endSponsorship, &beginSponsorship, &manageDataProductID, &endSponsorship,
					&beginSponsorship, &manageDataAppAccount, &endSponsorship,
					&beginSponsorship, &manageDataTypeName, &endSponsorship, &beginSponsorship, &manageDataTimeStamp, &endSponsorship, &beginSponsorship, &manageDataFromIdentifier, &endSponsorship},
				BaseFee:       constants.MinBaseFee,
	...
	sposorerSK := decrpytIssuerSecretKey1
	sponsorerKeypair, _ := keypair.ParseFull(sposorerSK)

	txe64, err := tx1.Sign(commons.GetStellarNetwork(), sponsorerKeypair)
	if err != nil {
